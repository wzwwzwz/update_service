...
(function ($) {
    $.fn.cmzSelect = function ({
        ...
        // arr         下拉列表数据源
        // label       input展示数据字段
        // callback    选中项的回调函数; callback(b_new, o_li)  // 是否为新创建; 当为false时为选项
        // searchList  模糊搜索的字段
        // liRow         自定义下拉列表dom
      ...
      <ul class="list"></ul>
      <ul class="listT">
      <li id="new_opt"> 无匹配数据; <span style='color:blue'>创建新的</span> </li>
      </ul>
        ...
        if (null === liRow) {
            ...
            arr.forEach(row => {
                ...
                for (let child of liRow) {
                    ...
                    if (false === flag) {
                        w += Number(childDom.style.width.split("p")[0]);
                    }
                }
                if (!flag) {
                    flag = !flag;
        ...
        // 列表hover时，切换背景
        $listT.find("li").on("mouseenter mouseleave", function (e) {
            $(this).toggleClass("on");
            // 阻止事件传递
            e.stopPropagation();
        });
        ...
        $cmzInput.on("blur", function () {
            ...
            let flagT = $listT.find("li").hasClass("on");
            // inputVal = $cmzInput.val();
                ...
                $lis.each(function (i, v) {
                    ...
                    if (isType(searchList, "Array") && searchList.length) {
                        ...
                        flag = searchList.some(key =>
                            rowObj[key]
                                ? -1 !== rowObj[key].toLowerCase().indexOf(val.toLowerCase())
                                : false
                        );
                    } else {
                        flag = -1 !== contentValue.toLowerCase().indexOf(val.toLowerCase());
                    }
